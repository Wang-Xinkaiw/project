...
class ImprovedADMMStrategy(BaseTuningStrategy):
    def __init__(self):
        self.min_beta = 1e-4
        self.max_beta = 1e6
        self.mu = 12.0  # 增加阈值，减少调整频率
        self.tau_inc = 1.8  # 增加增量因子，使调整更平缓
        self.tau_dec = 2.0  # 减小减量因子，使调整更平滑
    ...
    def update_parameters(self, iteration_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 增加最小迭代次数，确保在稳定阶段才进行调整
        min_iter = 50
        
        if iteration_count < min_iter and dual_res < 1e-10:
        ...
        if dual_res > 1e-10:
            ...
            # 调整阈值，使其更加保守
            if ratio > self.mu * 0.9:  # 稍微降低阈值，更容易触发调整
                new_beta = current_beta * self.tau_inc
            elif ratio < 1.0 / (self.mu * 1.1):  # 稍微提高阈值，更难触发减小
