...
class ImprovedADMMStrategy(BaseTuningStrategy):
    def __init__(self):
        ...
        self.smoothing_factor = 0.3  # 增大平滑因子以允许更稳定调整
        self.min_convergence_iter = 50  # 添加最小收敛迭代次数
    ...
    def update_parameters(self, iteration_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 使用平滑因子避免大幅调整
        if dual_res > 1e-10:
            if iteration < self.min_convergence_iter:
                # 在收敛早期使用更平滑的调整
                mu = self.mu * 0.8  # 早期收敛阈值更保守
            else:
                mu = self.mu
                
            if adjusted_ratio > mu:
                # 更保守地增加beta
                new_beta = current_beta * self.tau_inc
            elif adjusted_ratio < 1.0 / mu:
                # 更保守地减少beta
    ...
    def get_parameters(self) -> Dict[str, Any]:
        return {
            ...
            'min_convergence_iter': self.min_convergence_iter
        }
    ...
    def set_parameters(self, params: Dict[str, Any]) -> None:
        ...
        if 'min_convergence_iter' in params:
            self.min_convergence_iter = params['min_convergence_iter']