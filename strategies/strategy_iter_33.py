...
class ImprovedADMMStrategy(BaseTuningStrategy):
    def __init__(self):
        self.min_beta = 1e-3
        self.max_beta = 1e6
        self.mu = 9.3  # 从9.0调整到9.3
        self.tau_inc = 1.7
        self.tau_dec = 1.9
        self.min_beta_threshold = 0.1
        self.smoothing_factor = 0.9  # 新增平滑因子，用于调整残差比值
    ...
    def update_parameters(self, iteration_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if dual_res > 1e-10:
            # 引入平滑因子，避免调整过于敏感
            smoothed_ratio = primal_res / dual_res * self.smoothing_factor
            
            if smoothed_ratio > self.mu * 0.8:
                new_beta = current_beta * self.tau_inc
            elif smoothed_ratio < 1.0 / (self.mu * 0.8):
                # 保守调整：避免beta过快下降
