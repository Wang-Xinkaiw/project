...
class ImprovedADMMStrategy(BaseTuningStrategy):
    def __init__(self):
        ...
        self.max_beta = 1e6
        self.mu = 10.0  # 从12.0降低到10.0，使调整更保守
        self.tau_inc = 1.5  # 从1.8降低到1.5，使增加因子更小
        self.tau_dec = 2.0  # 从2.2降低到2.0，使减少因子更平滑
        self.min_iter = 30  # 增加最小迭代次数，让算法在更多迭代中稳定下来
    ...
    def update_parameters(self, iteration_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if dual_res > 1e-10:
            ...
            if adjusted_ratio > mu:
                ...
                # 稍微更保守的平滑调整
                new_beta = current_beta * (1.0 + 0.05)  # 减小平滑因子的影响
        else:
